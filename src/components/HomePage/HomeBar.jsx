...
export default function HomeBar() {
  ...
  return (
      ...
      <div className="content">
        ...
        <Box className="cards-container">
          {cardsData.map((card, index) => (
            // <div key={index} className="card" style={{ backgroundImage: `url(${card.url})` }}>
            <div key={cardsData.id} className="card" style={{ backgroundImage: `url(${card.url})` }}>
              <div className="card-content">
                {/* <h1 className="card-image">{<img src={card.url}/>}</h1> */}
                <h2 className="card-title">{card.title}</h2>
                <p className="card-body">{card.description}</p>
                <IconButton aria-label="like" onClick={() => handleLike(index)} style={{ backgroundColor: 'white', marginRight:'8px' }}>
    ...
    {/* <Box><Footer className="footer"/></Box> */}
    </>
  );
}
